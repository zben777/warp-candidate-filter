...
__global__ __launch_bounds__(BLOCK_SIZE) void v5_block_coalesced_store_kernel(
    ...
    int* __restrict__ output_count, int* __restrict__ output_checksum)
{
    ...
    //
    // ...
    int block_result_base = warp_id * NUM_B_LISTS;
    // Load A once。
...
#pragma unroll
    for (int b_id = 0; b_id < NUM_B_LISTS; ++b_id) {
        ...
        if (lane == 0) {
            int local_result_idx = block_result_base + b_id;
            s_output_count[local_result_idx] = count;
            s_output_checksum[local_result_idx] = checksum;
        }
    }
    ...
    if (tx < RESULTS_PER_BLOCK) {
        ...
        output_count[global_result_idx] = s_output_count[tx];
        output_checksum[global_result_idx] = s_output_checksum[tx];
    }
}
...
int main()
{
    ...
    for (int i = 0; i < WARMUP; ++i) {
        v5_block_coalesced_store_kernel<<<NUM_BLOCKS, BLOCK_SIZE>>>(
    ...
    printf("\n");
    printf("V5 1A-NB V1 + Block-Coalesced Output Store\n");
