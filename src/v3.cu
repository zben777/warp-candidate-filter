...
__device__ __forceinline__ bool warp_fixed32_membership(int sorted_a,
                                                        int target)
{
    int pos = -1;
    // Step +16
...
//
// V3 只替换 Search。
__global__ __launch_bounds__(BLOCK_SIZE) void v3_fixed32_kernel(
...
#pragma unroll
    for (int b_id = 0; b_id < NUM_B_LISTS; ++b_id) {
        ...
        // 改成：
        // Fixed-32 specialized search
        bool duplicated = warp_fixed32_membership(sorted_a, b);
        // 后面的 compact 与 V1 相同。
        int count;
        int checksum;
...
int main()
{
    ...
    for (int i = 0; i < WARMUP; ++i) {
        v3_fixed32_kernel<<<NUM_BLOCKS, BLOCK_SIZE>>>(d_a, d_b, d_count,
                                                      d_checksum);
    }
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    printf("\n");
    printf("V3 1A-NB Sort Once + Fixed-32 Search\n");
