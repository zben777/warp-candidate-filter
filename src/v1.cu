...
__device__ __forceinline__
unsigned bfe(unsigned lane_id, unsigned pos)
{
  unsigned res;
...
void binary_search_real_kernel(
...
int main()
{
    ...
    for (int b_id = 0;
      ...
      if (h_count[idx] != 16 ||
        ...
        if (errors < 10) {
          ...
          printf(
              "Mismatch group=%zu b=%d count=%d checksum=%d expected_checksum=%d\n",
              group,
              b_id,
              h_count[idx],
              h_checksum[idx],
  ...
  printf("\n");
  printf(
      "V1-real 1A-NB Sort A Once + Binary Search\n");
