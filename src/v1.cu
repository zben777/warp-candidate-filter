...
{
    int y = __shfl_xor_sync(0xffffffffu, x, mask, WARP_SIZE);
    return ((x < y) == dir) ? y : x;
}
...
#pragma unroll
    for (int step = 0; step < 6; ++step) {
        ...
        int value = __shfl_sync(0xffffffffu, sorted_a, mid);
        if (active) {
            if (value == target) {
                existed = true;
...
__global__ __launch_bounds__(BLOCK_SIZE) void binary_search_real_kernel(
    ...
    int* __restrict__ output_count, int* __restrict__ output_checksum)
{
    ...
    // A 只从 Global Memory 读一次。
    int a_value = input_a[group_id * WARP_SIZE + lane];
    // 核心：
    // A 只排序一次。
    //
    // 后续所有 B 都复用
    // 当前 Warp registers 中的 sorted A。
...
#pragma unroll
    for (int b_id = 0; b_id < NUM_B_LISTS; ++b_id) {
        ...
        int b = input_b[b_idx];
        // 当前 B[lane]
        // 在同一个 sorted A 中进行 Binary Search。
...
int main()
{
    ...
    for (int i = 0; i < WARMUP; ++i) {
        binary_search_real_kernel<<<NUM_BLOCKS, BLOCK_SIZE>>>(d_a, d_b, d_count,
                                                              d_checksum);
    }
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    // 与 V0-real 完全相同的逻辑 Global traffic。
    //
    // A input:
    // 128 B
    //
    ...
    printf("\n");
    printf("V1-real 1A-NB Sort A Once + Binary Search\n");
