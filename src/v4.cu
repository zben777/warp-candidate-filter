...
warp_fixed32_membership_2way(int sorted_a, int target0, int target1,
                             bool& existed0, bool& existed1)
{
    int pos0 = -1;
    int pos1 = -1;
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    {
        ...
        int probe1 = pos1 + 16;
        // 两个独立的 SHFL
        int value0 = __shfl_sync(FULL_MASK, sorted_a, probe0, WARP_SIZE);
        int value1 = __shfl_sync(FULL_MASK, sorted_a, probe1, WARP_SIZE);
        pos0 = (value0 < target0) ? probe0 : pos0;
        pos1 = (value1 < target1) ? probe1 : pos1;
    }
    // --------------------------------------------------------
    // stride = 8
    // --------------------------------------------------------
    ...
    // candidate 一定位于 [0,31]
    // --------------------------------------------------------
    int candidate0 = pos0 + 1;
    int candidate1 = pos1 + 1;
    // 仍然先连续发两个独立 SHFL。
    int final_value0 = __shfl_sync(FULL_MASK, sorted_a, candidate0, WARP_SIZE);
    int final_value1 = __shfl_sync(FULL_MASK, sorted_a, candidate1, WARP_SIZE);
    existed0 = (final_value0 == target0);
    existed1 = (final_value1 == target1);
}
...
__device__ __forceinline__ bool warp_fixed32_membership_1way(int sorted_a,
                                                             int target)
{
    int pos = -1;
    {
        int probe = pos + 16;
        int value = __shfl_sync(FULL_MASK, sorted_a, probe, WARP_SIZE);
        pos = (value < target) ? probe : pos;
    }
    {
        int probe = pos + 8;
        int value = __shfl_sync(FULL_MASK, sorted_a, probe, WARP_SIZE);
    ...
    int candidate = pos + 1;
    int value = __shfl_sync(FULL_MASK, sorted_a, candidate, WARP_SIZE);
    return value == target;
}
...
__global__ __launch_bounds__(BLOCK_SIZE) void v4_fixed32_2way_ilp_kernel(
...
#pragma unroll
    for (int b_base = 0; b_base + 1 < NUM_B_LISTS; b_base += 2) {
        ...
        // V2 2-way ILP
        // ----------------------------------------------------
        warp_fixed32_membership_2way(sorted_a, b0, b1, duplicated0,
                                     duplicated1);
        // ----------------------------------------------------
        // B0 Compact
        //
    ...
    size_t b_idx = (group_id * NUM_B_LISTS + b_id) * WARP_SIZE + lane;
    int b = input_b[b_idx];
    bool duplicated = warp_fixed32_membership_1way(sorted_a, b);
    int count;
    int checksum;
...
int main()
{
    ...
    for (int i = 0; i < WARMUP; ++i) {
        v4_fixed32_2way_ilp_kernel<<<NUM_BLOCKS, BLOCK_SIZE>>>(
    ...
    // --------------------------------------------------------
    printf("\n");
    printf("V4 1A-NB Fixed-32 + 2-Way B ILP\n");
    ...
    printf("Search probes   : %d / group\n", 15 + NUM_B_LISTS * 6);
    printf("Search type     : Fixed-32\n");
